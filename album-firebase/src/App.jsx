...
import {
  createUser, updateUserCollection, subscribeToUsers
} from "./firebase";
...
function calcStats(col) {
  ...
  return {total,owned,missing,pct,reps,sections,
    complete:sorted.filter(s=>s.pct===100),
...
function findMatches(users, myId) {
  ...
  // helper: does this user effectively have this sticker?
  const hasSticker=(col,sk,n)=> col[sk]?.[n]?.owned || (col[sk]?.[n]?.repeated||0)>0;
  const hasRepeat =(col,sk,n)=> (col[sk]?.[n]?.repeated||0)>0;
  const needsSticker=(col,sk,n)=> !hasSticker(col,sk,n);
  ...
  return Object.entries(users)
    ...
    .map(([uid,user])=>{
      ...
      TEAMS.forEach(t=>{
        TEAM_STICKERS.forEach(n=>{
          // I can give: I have repeated AND they need it
          if(hasRepeat(myCol,t,n) && needsSticker(theirCol,t,n))
            gives.push({section:t,num:n,flag:TEAM_FLAGS[t]});
          // They can give: they have repeated AND I need it
          if(hasRepeat(theirCol,t,n) && needsSticker(myCol,t,n))
            needs.push({section:t,num:n,flag:TEAM_FLAGS[t]});
        });
      });
      SPECIAL_STICKERS.forEach(n=>{
        if(hasRepeat(myCol,"ESPECIALES",n) && needsSticker(theirCol,"ESPECIALES",n))
          gives.push({section:"ESPECIALES",num:n,flag:"✨"});
        if(hasRepeat(theirCol,"ESPECIALES",n) && needsSticker(myCol,"ESPECIALES",n))
          needs.push({section:"ESPECIALES",num:n,flag:"✨"});
      });
      COCA_STICKERS.forEach(n=>{
        if(hasRepeat(myCol,"CC",n) && needsSticker(theirCol,"CC",n))
          gives.push({section:"CC",num:n,flag:"🥤"});
        if(hasRepeat(theirCol,"CC",n) && needsSticker(myCol,"CC",n))
          needs.push({section:"CC",num:n,flag:"🥤"});
      });
      if(gives.length>0||needs.length>0)
        return {uid,name:user.name,avatar:user.avatar,color:user.color,gives,needs};
      return null;
    })
    .filter(Boolean);
}
...
function Avatar({avatar,color,size=36}) {
  return (
    <div style={{width:size,height:size,borderRadius:"50%",background:color?.bg||"#1a56db",
...
function ProgressBar({pct,color="#057a55",height=6}) {
  return (
    <div style={{background:"#e5e7eb",borderRadius:99,height,overflow:"hidden",width:"100%"}}>
      <div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:99,transition:"width .35s"}}/>
    </div>
  );
}
...
function StickerMenu({val,label,onToggle,onAddRepeat,onRemoveRepeat,onRemoveOwned,onClose}) {
  ...
  const Btn=({onClick,bg,col,children})=>(
    <button onClick={onClick} style={{display:"block",width:"100%",padding:"7px 10px",marginBottom:4,
      background:bg,color:col,border:"none",borderRadius:8,cursor:"pointer",fontSize:13,
      textAlign:"left",fontWeight:500}}>
      {children}
    </button>
  );
  return (
    <div ref={ref} style={{position:"absolute",zIndex:9999,top:42,left:0,
      background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:10,
      boxShadow:"0 8px 32px rgba(0,0,0,0.15)",minWidth:220,whiteSpace:"nowrap"}}>
      <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:8,paddingLeft:4}}>{label}</div>
      {/* Toggle owned */}
      <Btn onClick={onToggle} bg={val.owned?"#fde8e8":"#def7ec"} col={val.owned?"#991b1b":"#065f46"}>
        {val.owned?"❌ Marcar como faltante":"✅ Marcar como obtenido"}
      </Btn>
      {/* Only show repeat options if already owned */}
      {val.owned&&(
      ...
      {val.owned&&(
        <Btn onClick={onRemoveOwned} bg="#f3f4f6" col="#dc2626">🗑 Borrar (corregir error)</Btn>
      )}
    </div>
  );
}
...
function StickerCell({sectionKey,num,val,onToggle,onAddRepeat,onRemoveRepeat,onRemoveOwned}) {
  ...
  // repeated always implies owned — no "solo repetido" state
  const owned   = val?.owned || false;
  const repeated= val?.repeated || 0;
  ...
  return (
    <div style={{position:"relative",display:"inline-block"}}>
      ...
      <div onClick={()=>setOpen(o=>!o)} style={{width:34,height:34,borderRadius:8,background:bg,border,
        display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",
        position:"relative",userSelect:"none",fontSize:10,fontWeight:700,color:tc}}>
        {repeated>0&&(
          <span style={{position:"absolute",top:-6,right:-6,background:"#d97706",color:"#fff",
            borderRadius:"50%",width:16,height:16,fontSize:9,display:"flex",alignItems:"center",
            justifyContent:"center",fontWeight:700,zIndex:1}}>{repeated}</span>
        )}
        {owned?"✓":"–"}
      </div>
      {open&&(
        <StickerMenu val={val||{owned:false,repeated:0}} label={label}
...
export default function App() {
  ...
  const [myId,setMyId]             = useState(()=>localStorage.getItem("album_myId")||null);
  ...
  // ── persist collection to firebase (debounced 800ms) ──
  const flushCollection = useCallback((col)=>{
    if(!myId) return;
    if(saveTimer.current) clearTimeout(saveTimer.current);
    setSaving(true);
    saveTimer.current = setTimeout(async()=>{
      try { await updateUserCollection(myId, col); }
  ...
  // Adding a repeat always marks the sticker as owned too
  ...
  const handleJoinRoom=()=>{
    if(roomInput.trim().toUpperCase()!==ROOM_CODE){setRoomError("Código incorrecto. Pídelo a quien te invitó.");return;}
  ...
  const handleRegister=async()=>{
    ...
    const data={name:newName.trim(),avatar:newAvatar,color:newColor,
  ...
  const handleLogout=()=>{
    ...
    setMyId(null);setScreen("login");setTab("album");
  };

  const filteredTeams = secSearch
    ? TEAMS.filter(t=>t.toLowerCase().includes(secSearch.toLowerCase())
        ||TEAM_NAMES[t]?.toLowerCase().includes(secSearch.toLowerCase()))
    : TEAMS;
  ...
  const s={
    ...
    center:{maxWidth:420,margin:"0 auto",padding:"40px 16px"},
    card:{background:"#fff",borderRadius:16,padding:20,border:"1px solid #e5e7eb",marginBottom:12},
    input:{width:"100%",fontSize:15,padding:"10px 14px",border:"1px solid #d1d5db",borderRadius:10,
      outline:"none",boxSizing:"border-box"},
    btnPrimary:{width:"100%",padding:13,fontSize:15,fontWeight:700,background:"#1a56db",
  ...
  if(screen==="loading") return (
    <div style={{...s.page,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontSize:14,color:"#6b7280"}}>Conectando...</div>
      </div>
    </div>
  );
      ...
      <div style={{width:"100%",maxWidth:400}}>
        ...
        <div style={s.card}>
          ...
          <input value={roomInput} onChange={e=>setRoomInput(e.target.value)}
            ...
            style={{...s.input,letterSpacing:3,fontWeight:700,textTransform:"uppercase"}}/>
          {roomError&&<p style={{color:"#dc2626",fontSize:12,marginTop:8,marginBottom:0}}>{roomError}</p>}
        </div>
        <button onClick={handleJoinRoom} style={s.btnPrimary}>Entrar →</button>
        ...
        {Object.keys(users).length>0&&(
          <div style={{marginTop:28}}>
            ...
            {Object.entries(users).map(([id,u])=>{
              ...
              return (
                <div key={id} onClick={()=>handlePickUser(id)}
                  style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",
                    background:"#fff",borderRadius:12,marginBottom:8,cursor:"pointer",border:"1px solid #e5e7eb"}}>
                  <Avatar avatar={u.avatar} color={u.color} size={38}/>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600,fontSize:14}}>{u.name}</div>
                    <div style={{fontSize:12,color:"#6b7280"}}>{st?.pct||0}% completado · {st?.owned||0}/{st?.total||994}</div>
                  </div>
    ...
    <div style={{...s.page,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 16px"}}>
      <div style={{width:"100%",maxWidth:400}}>
        ...
        <div style={{marginBottom:16}}>
          <label style={s.label}>Tu nombre o apodo</label>
        ...
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {AVATARS.map(a=>(
              <div key={a} onClick={()=>setNewAvatar(a)}
                style={{width:42,height:42,borderRadius:10,display:"flex",alignItems:"center",
                  justifyContent:"center",fontSize:22,cursor:"pointer",
                  border:newAvatar===a?"2px solid #1a56db":"1px solid #e5e7eb",
                  background:newAvatar===a?"#e8f0fe":"#fff"}}>
                {a}
              </div>
            ))}
          </div>
        ...
        <div style={{marginBottom:24}}>
          <label style={s.label}>Elige tu color</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {COLORS.map(c=>(
              <div key={c.name} onClick={()=>setNewColor(c)} title={c.name}
                style={{width:34,height:34,borderRadius:"50%",background:c.bg,cursor:"pointer",
                  border:newColor.name===c.name?"3px solid #111":"3px solid transparent"}}/>
            ))}
          </div>
        </div>

        <div style={{display:"flex",alignItems:"center",gap:12,background:"#f3f4f6",
        ...
        <button onClick={handleRegister} disabled={!newName.trim()}
          style={{...s.btnPrimary,background:newName.trim()?"#1a56db":"#d1d5db",
  ...
  return (
    <div style={s.page}>
        ...
        <div style={{maxWidth:820,margin:"0 auto",padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:52}}>
          ...
          {me&&(
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {alerts.length>0&&(
                <span style={{background:"#fef3c7",color:"#92400e",fontSize:11,fontWeight:600,
                  padding:"3px 9px",borderRadius:99,border:"1px solid #fcd34d"}}>
                  🔔 {alerts.length} match{alerts.length>1?"es":""}
                </span>
              )}
              <div onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:8,
                padding:"4px 10px",border:"1px solid #e5e7eb",borderRadius:20,cursor:"pointer"}}>
      ...
      {tab!=="intercambios"&&alerts.slice(0,2).map(m=>(
        <div key={m.uid} style={{background:"#fef3c7",borderBottom:"1px solid #fcd34d",
          ...
          <button onClick={()=>setTab("intercambios")}
            style={{fontSize:11,fontWeight:600,background:"#d97706",color:"#fff",
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #e5e7eb"}}>
        <div style={{maxWidth:820,margin:"0 auto",display:"flex",padding:"0 16px",overflowX:"auto"}}>
          {[["album","📋 Álbum"],["stats","📊 Estadísticas"],["intercambios","🔄 Intercambios"],["usuarios","👥 Usuarios"]].map(([key,lbl])=>(
            <button key={key} onClick={()=>setTab(key)} style={{padding:"12px 14px",fontSize:13,
              fontWeight:tab===key?700:400,background:"none",border:"none",whiteSpace:"nowrap",
              borderBottom:tab===key?"2px solid #1a56db":"2px solid transparent",
              color:tab===key?"#1a56db":"#6b7280",cursor:"pointer"}}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:16}}>
        ...
        {tab==="album"&&me&&(
          <div>
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              ...
              {[["#def7ec","#065f46","✓ Obtenido"],["#fef3c7","#92400e","✓+🔁 Con repetidos"],["#f3f4f6","#9ca3af","– Falta"]].map(([bg,col,lbl])=>(
            ...
            <input value={secSearch} onChange={e=>setSecSearch(e.target.value)}
              ...
              style={{width:"100%",fontSize:13,padding:"8px 12px",border:"1px solid #d1d5db",
                borderRadius:10,outline:"none",boxSizing:"border-box",marginBottom:12}}/>

            <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:16}}>
              {filteredTeams.map(t=>{
                const sv=stats?.sections[t], active=selSection===t;
                return (
                  <button key={t} onClick={()=>setSelSection(t)} style={{padding:"5px 9px",
                    fontSize:11,fontWeight:600,borderRadius:8,cursor:"pointer",
                    border:active?"2px solid #1a56db":"1px solid #d1d5db",
                    background:active?"#e8f0fe":"#fff",color:active?"#1a56db":"#374151",position:"relative"}}>
                    {TEAM_FLAGS[t]} {t}
                    {sv?.pct===100&&<span style={{position:"absolute",top:-5,right:-5,fontSize:8,
                      background:"#057a55",color:"#fff",borderRadius:99,padding:"1px 4px"}}>✓</span>}
                  </button>
                );
              })}
              {["ESPECIALES","CC"].map(key=>{
                const active=selSection===key;
                const style_={padding:"5px 9px",fontSize:11,fontWeight:600,borderRadius:8,cursor:"pointer",
                  border:active?"2px solid "+(key==="ESPECIALES"?"#7e3af2":"#e74694"):"1px solid #d1d5db",
                  background:active?(key==="ESPECIALES"?"#edebfe":"#fce7f3"):"#fff",
                  color:active?(key==="ESPECIALES"?"#7e3af2":"#e74694"):"#374151"};
                return <button key={key} onClick={()=>setSelSection(key)} style={style_}>{key==="ESPECIALES"?"✨ Especiales":"🥤 Coca-Cola"}</button>;
              })}
            </div>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:16,border:"1px solid #e5e7eb"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    <div style={{fontWeight:700,fontSize:15}}>
                      {isEsp?"Especiales":isCC?"Coca-Cola":(TEAM_NAMES[selSection]||selSection)}
                      {TEAMS.includes(selSection)&&<span style={{fontWeight:400,color:"#9ca3af",fontSize:12}}> ({selSection})</span>}
                    </div>
                    <div style={{fontSize:12,color:"#6b7280"}}>{sInfo?.owned}/{sInfo?.total} obtenidos · {sInfo?.reps} repetidos</div>
                  </div>
                </div>
                <div style={{fontSize:20,fontWeight:700,
                  color:sInfo?.pct===100?"#057a55":sInfo?.pct>50?"#1a56db":"#d97706"}}>
                  {sInfo?.pct}%
                </div>
              </div>
              <ProgressBar pct={sInfo?.pct||0}
                color={sInfo?.pct===100?"#057a55":sInfo?.pct>50?"#1a56db":"#d97706"} height={6}/>
              <div style={{marginTop:14,display:"flex",flexWrap:"wrap",gap:6}}>
                {stickerList.map(n=>(
                  <StickerCell key={n} sectionKey={selSection} num={n}
                    val={me.collection[selSection]?.[n]}
                    onToggle={()=>onToggle(selSection,n)}
              ...
              <p style={{fontSize:11,color:"#9ca3af",marginTop:12,marginBottom:0}}>
                Haz clic en un cromo para ver las opciones
              </p>
            </div>
          </div>
        )}
        ...
        {tab==="stats"&&stats&&(
          <div>
            <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
              <StatCard label="Completado" value={`${stats.pct}%`} sub={`${stats.owned} de ${stats.total} cromos`}
                color={stats.pct>75?"#057a55":stats.pct>40?"#1a56db":"#d97706"}/>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:16,border:"1px solid #e5e7eb",marginBottom:16}}>
              ...
              {[
                {lbl:`Selecciones (${TEAMS.length})`,owned:TEAMS.reduce((a,t)=>a+(stats.sections[t]?.owned||0),0),total:TEAMS.length*20,color:"#1a56db"},
                {lbl:"✨ Especiales",owned:stats.sections["ESPECIALES"]?.owned||0,total:20,color:"#7e3af2"},
                {lbl:"🥤 Coca-Cola",owned:stats.sections["CC"]?.owned||0,total:14,color:"#e74694"},
              ].map(row=>(
                <div key={row.lbl} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:4}}>
                    <span style={{fontWeight:500}}>{row.lbl}</span>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              {[
                {title:"🟢 Más avanzadas",data:stats.best,color:"#057a55"},
                {title:"🔴 Más incompletas",data:stats.worst,color:"#dc2626"},
                ...
                <div key={title} style={{background:"#fff",borderRadius:14,padding:14,border:"1px solid #e5e7eb"}}>
                  ...
                  {data.map(sv=>(
                    <div key={sv.code} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                      <span style={{fontSize:14}}>{TEAM_FLAGS[sv.code]}</span>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,fontWeight:600}}>{sv.code} <span style={{fontWeight:400,color:"#6b7280"}}>{sv.pct}%</span></div>
            ...
            {stats.complete.length>0&&(
              <div style={{background:"#def7ec",border:"1px solid #6ee7b7",borderRadius:14,padding:14,marginBottom:16}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {stats.complete.map(sv=>(
                    <span key={sv.code} style={{background:"#057a55",color:"#fff",padding:"3px 11px",borderRadius:99,fontSize:12,fontWeight:600}}>
                      {TEAM_FLAGS[sv.code]} {sv.code}
                    </span>
                  ))}
                </div>
              </div>
              ...
              {TEAMS.map(t=>{
                ...
                return (
                  <div key={t} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                    ...
                    {sv.reps>0&&<span style={{fontSize:10,background:"#fef3c7",color:"#92400e",padding:"1px 6px",borderRadius:99,whiteSpace:"nowrap"}}>{sv.reps}× rep.</span>}
                  </div>
                );
              })}
            </div>
        ...
        {tab==="intercambios"&&(
          <div>
            <div style={{background:"#e8f0fe",border:"1px solid #93c5fd",borderRadius:12,
              padding:"10px 16px",marginBottom:16,fontSize:13,color:"#1e40af"}}>
              🔄 Los matches se detectan <strong>en tiempo real</strong>. Cuando alguien agrega un repetido que tú necesitas, aparece aquí al instante.
            </div>
            ...
            {matches.length===0?(
              <div style={{textAlign:"center",padding:"48px 20px",color:"#9ca3af"}}>
                ...
                <div style={{fontSize:13,marginTop:6}}>Cuando más usuarios registren sus repetidos, aparecerán aquí automáticamente.</div>
              </div>
            ):matches.map(m=>(
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:"#6b7280"}}>
                      {m.gives.length} que tú le das · {m.needs.length} que ellos te dan
                    </div>
                  </div>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontSize:20,fontWeight:700,color:"#1a56db"}}>{m.gives.length+m.needs.length}</div>
                ...
                {m.gives.length>0&&(
                  <div style={{marginBottom:10,background:"#f0fdf4",borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#065f46",marginBottom:6}}>
                      ✅ Tú le puedes dar a {m.name} ({m.gives.length}):
                    </div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {m.gives.slice(0,30).map((g,i)=>(
                        <span key={i} style={{fontSize:11,background:"#def7ec",color:"#065f46",
                          padding:"2px 8px",borderRadius:99,fontWeight:500}}>
                          {g.flag} {g.section==="ESPECIALES"?"Esp":g.section}#{g.num}
                        </span>
                      ))}
                      {m.gives.length>30&&<span style={{fontSize:11,color:"#9ca3af"}}>+{m.gives.length-30} más</span>}
                    </div>
                  </div>
                )}
                ...
                {m.needs.length>0&&(
                  <div style={{background:"#eff6ff",borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#1e40af",marginBottom:6}}>
                      🎯 {m.name} te puede dar a ti ({m.needs.length}):
                    </div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {m.needs.slice(0,30).map((g,i)=>(
                        <span key={i} style={{fontSize:11,background:"#e8f0fe",color:"#1e40af",
                          padding:"2px 8px",borderRadius:99,fontWeight:500}}>
                          {g.flag} {g.section==="ESPECIALES"?"Esp":g.section}#{g.num}
                        </span>
                      ))}
                      {m.needs.length>30&&<span style={{fontSize:11,color:"#9ca3af"}}>+{m.needs.length-30} más</span>}
                    </div>
                  </div>
                )}
              </div>
        ...
        {tab==="usuarios"&&(
          <div>
            ...
            <div style={{fontSize:13,color:"#6b7280",marginBottom:12}}>
              {Object.keys(users).length} usuario(s) en la sala · datos en tiempo real 🟢
            </div>

            {Object.entries(users)
              .sort(([,a],[,b])=>calcStats(b.collection).pct-calcStats(a.collection).pct)
              .map(([id,u],i)=>{
                const sv=calcStats(u.collection);
                const isMe=id===myId;
                return (
                  <div key={id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",
                    background:"#fff",borderRadius:12,marginBottom:8,
                    border:isMe?"2px solid #1a56db":"1px solid #e5e7eb"}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                        ...
                        {isMe&&<span style={{fontSize:10,background:"#1a56db",color:"#fff",
                          padding:"1px 8px",borderRadius:99,fontWeight:600}}>tú</span>}
                      </div>
                      <div style={{fontSize:12,color:"#6b7280"}}>
                        {sv.owned}/{sv.total} cromos · {sv.reps} repetidos · {sv.complete.length} secc. completas
                      </div>
                      <div style={{marginTop:4}}>
                        <ProgressBar pct={sv.pct} color={u.color?.bg||"#1a56db"} height={4}/>
                      </div>
                    </div>
                    <div style={{fontSize:22,fontWeight:700,color:u.color?.bg||"#1a56db"}}>{sv.pct}%</div>
                  </div>
                );
              })}
          </div>
