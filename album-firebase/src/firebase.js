...
// Predictions: /rooms/{roomId}/predictions/{matchId} -> { userId: {homeScore, awayScore, name}, _result: {homeScore, awayScore} }
export async function savePrediction(roomId, matchId, userId, pred) {
  const ref = doc(db, "rooms", roomId, "predictions", matchId);
  await setDoc(ref, { [userId]: pred }, { merge: true });
}
export async function saveMatchResult(roomId, matchId, result) {
  const ref = doc(db, "rooms", roomId, "predictions", matchId);
  await setDoc(ref, { _result: result }, { merge: true });
}
export function subscribeToPredictions(roomId, cb) {
  return onSnapshot(collection(db, "rooms", roomId, "predictions"), snap=>{
    const data={};
