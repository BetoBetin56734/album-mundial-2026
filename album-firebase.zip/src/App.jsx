...
function findMatches(users, myId) {
  ...
  return Object.entries(users)
    ...
    .map(([uid,user])=>{
      ...
      TEAMS.forEach(t=>{
        TEAM_STICKERS.forEach(n=>{
          const myS=myCol[t]?.[n], thS=theirCol[t]?.[n];
      ...
      SPECIAL_STICKERS.forEach(n=>{
        const myS=myCol["ESPECIALES"]?.[n],thS=theirCol["ESPECIALES"]?.[n];
      ...
      COCA_STICKERS.forEach(n=>{
        const myS=myCol["CC"]?.[n],thS=theirCol["CC"]?.[n];
...
function StickerMenu({val,label,onToggle,onAddRepeat,onRemoveRepeat,onRemoveOwned,onClose}) {
  ...
  return (
    <div ref={ref} style={{position:"absolute",zIndex:9999,top:42,left:0,
      ...
      boxShadow:"0 8px 32px rgba(0,0,0,0.15)",minWidth:210,whiteSpace:"nowrap"}}>
...
function StickerCell({sectionKey,num,val,onToggle,onAddRepeat,onRemoveRepeat,onRemoveOwned}) {
  ...
  else if(repeated>0)   {bg="#fce7f3";border="1px solid #e74694";tc="#9d174d";}
        ...
        {tab==="album"&&me&&(
          <div>
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              ...
              {[["#def7ec","#065f46","✓ Obtenido"],["#fef3c7","#92400e","🔁 +Repetido"],["#fce7f3","#9d174d","R Solo repetido"],["#f3f4f6","#9ca3af","– Falta"]].map(([bg,col,lbl])=>(
